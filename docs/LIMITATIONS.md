...
Observability remains a limitation: **No complete product-defined centralized error-monitoring, uptime-alerting, incident-response, log-retention, or PII-redaction policy has yet been implemented and verified.** Provider/platform logging cannot be guaranteed absent. Version 24 will define privacy-safe observability boundaries before telemetry integration; see [`VERSION_24_RELEASE_DIRECTION.md`](VERSION_24_RELEASE_DIRECTION.md). No unrestricted session replay is approved, and no raw résumé/job text should be intentionally captured by future observability tooling.
