...
Selected `jobfit.cooperrobillard.com` as the approved frontend hostname because it is product-specific, understandable, broader than internships, and does not move or disrupt the `cooperrobillard.com` portfolio root. The Render backend remains `https://internship-fit-gap-analyzer.onrender.com` because the custom domain is frontend-only and the browser should continue using the same-origin Next.js `/api/analyze` proxy. Clerk is the highest-risk dependency because production domains, publishable keys, redirects, sessions, cookies, account portal behavior, OAuth callbacks, `authorizedParties`, and Supabase JWT behavior can all depend on hostname configuration. The default Vercel alias is preserved for deployment inspection and rollback access, but it is not promised as a second authenticated production domain because Clerk treats the Vercel and custom hostnames as different root domains. Overlapping UptimeRobot monitoring is safer than immediately replacing the existing frontend monitor because it distinguishes custom-domain DNS/certificate failures from deployment failures during transition. Verified planning facts: registrar Porkbun LLC, authoritative DNS provider Porkbun DNS, nameservers `curitiba.ns.porkbun.com`, `fortaleza.ns.porkbun.com`, `maceio.ns.porkbun.com`, and `salvador.ns.porkbun.com`; no public `jobfit.cooperrobillard.com` DNS answer; no public CAA answer at inspection time; no DNS or provider configuration changed. Exact Step 2: add `jobfit.cooperrobillard.com` to the correct Vercel production project, obtain the exact provider-generated DNS record, add only that record at Porkbun DNS, verify ownership and TLS, preserve the default Vercel hostname, and document sanitized results without changing Clerk yet.

## Version 25 Step 2 — Vercel domain and Porkbun DNS configuration

Documented the completed human-controlled attachment of `jobfit.cooperrobillard.com` to the existing Vercel production project for `cooperrobillard/internship-fit-gap-analyzer`, with Porkbun DNS retaining authority and only the exact `jobfit` CNAME `6baf70c9e185a618.vercel-dns-017.com.` added at a 600-second TTL. Recorded successful authoritative DNS verification across `curitiba.ns.porkbun.com`, `fortaleza.ns.porkbun.com`, `maceio.ns.porkbun.com`, and `salvador.ns.porkbun.com`, plus independent Cloudflare `1.1.1.1` and Google `8.8.8.8` verification; Vercel `Valid Configuration` status; Production assignment; HTTPS HTTP 200 with SSL verification result `0`; Let's Encrypt `CN=YR1` certificate coverage for the custom hostname; and a passing private-browser landing-page check. The existing `internship-fit-gap-analyzer.vercel.app` hostname remains attached, returns HTTP 200, and has no redirect configured. Clerk remains explicitly deferred; no other providers, code, metadata, RLS, public links, environment variables, Render, FastAPI/CORS, Sentry, UptimeRobot, Supabase, nameservers, root DNS records, or unrelated Porkbun records changed. Exact next step: Version 25 Step 3 — reconcile Clerk’s production-domain configuration for `jobfit.cooperrobillard.com`, add only Clerk-generated DNS records when required, reconcile the production publishable key and Vercel environment, redeploy safely, review redirects, cookies, sessions, account portal behavior, OAuth callbacks, `authorizedParties`, and Supabase JWT integration, and verify authentication without weakening security.

## Version 25 Step 3A - Clerk authorized parties guardrail

Added a small Clerk authorized-parties helper and wired it into the Next.js proxy so local development authorizes only the two local origins while all other environments fail closed to the custom production hostname and preserved default Vercel hostname. Documented that the next phase is human Clerk production-domain reconciliation; no provider, DNS, environment, Supabase, RLS, or deployment changes were made.

## Version 25 Step 3 — Clerk Production migration closeout

Documented the completed human-controlled Clerk Development-to-Production activation for `jobfit.cooperrobillard.com` as a documentation-only closeout. Recorded sanitized results for Clerk Production authentication, DNS, certificate deployment, Allowed Subdomains, Supabase Third-Party Auth, unchanged RLS, Vercel Production/Preview key-class separation, production redeployment at commit `540b2876b8b11f267bb02418f167c5eeeecfc855`, authentication/session checks, analysis and saved-analysis CRUD, structured profiles, two-user RLS isolation, automated Version 23 Playwright production QA, old-host fail-closed behavior, and rollback readiness. No runtime code, tests, dependencies, environment values, provider settings, DNS, Supabase, RLS, secrets, QA-user identifiers, screenshots, traces, local report contents, or private dashboard URLs were added. Remaining Version 25 work stays separately scoped and should not be marked complete by this Step 3 closeout.
