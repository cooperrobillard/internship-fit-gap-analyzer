...
function Section({ id, title, children }: { id: string; title: string; children: ReactNode }) {
  return (
    ...
    >
      <h2 id={`${id}-heading`} className="text-2xl font-semibold tracking-tight text-[var(--color-text)]">
        {title}
      </h2>
...
function InlineCode({ children }: { children: ReactNode }) {
  return <code className="rounded bg-[var(--color-surface-muted)] px-1 py-0.5 text-xs text-[var(--color-text)]">{children}</code>;
}
    ...
    <main className="app-shell-container flex-1 py-10 sm:py-14 lg:py-16">
      <header className="max-w-3xl">
        ...
        <p className="mt-5 max-w-2xl text-base leading-8 text-[var(--color-text-muted)] sm:text-lg">
          How analysis inputs, optional saved records, and current user controls work in this limited public beta.
        </p>
      </header>

      <section aria-labelledby="at-a-glance-heading" className="mt-10 max-w-3xl border-y border-[var(--color-divider)] py-5">
        <h2 id="at-a-glance-heading" className="text-sm font-semibold text-[var(--color-text)]">
          At a glance
        </h2>
        <dl className="mt-4 divide-y divide-[var(--color-divider)] text-sm leading-6 text-[var(--color-text-muted)] sm:text-base">
          <div className="py-3 sm:grid sm:grid-cols-[12rem_1fr] sm:gap-5">
            <dt className="font-semibold text-[var(--color-text)]">Current request</dt>
            <dd className="mt-1 sm:mt-0">Analysis text is used for the current request.</dd>
          </div>
          <div className="py-3 sm:grid sm:grid-cols-[12rem_1fr] sm:gap-5">
            <dt className="font-semibold text-[var(--color-text)]">Saving</dt>
            <dd className="mt-1 sm:mt-0">Running an analysis does not automatically save it.</dd>
          </div>
          <div className="py-3 sm:grid sm:grid-cols-[12rem_1fr] sm:gap-5">
            <dt className="font-semibold text-[var(--color-text)]">Saved records</dt>
            <dd className="mt-1 sm:mt-0">Optional saves contain structured skills and metadata.</dd>
          </div>
        </dl>
        <div className="mt-4 space-y-2 text-sm leading-6 text-[var(--color-text-muted)]">
          <p>Platform or service logging cannot be guaranteed absent. Avoid unusually sensitive information.</p>
          <p>
            This page is a product data-handling summary, not a formal legal privacy policy, compliance certification,
            security audit, or penetration-test report.
          </p>
        </div>
      </section>
      ...
      <div className="mt-10 grid gap-9 lg:grid-cols-[12rem_minmax(0,46rem)] lg:items-start">
        <aside className="lg:sticky lg:top-6">
          <nav aria-label="Privacy page sections">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--color-text-muted)]">On this page</p>
        ...
        <article className="min-w-0 space-y-10">
          <Section id="summary" title="Plain-language summary">
            ...
            <p>Platform or service logging cannot be guaranteed absent, so avoid unusually sensitive information.</p>
          </Section>

          <Section id="transient-inputs" title="Analysis inputs">
            <p>
              Pasted résumé and job-description text is used to produce the current analysis response. Plain <InlineCode>.txt</InlineCode> uploads are read as transient convenience inputs.
            </p>
            <p>
          ...
          <Section id="saved-analyses" title="Saved analyses">
            <p>When you explicitly save an analysis, the account-owned structured record may contain:</p>
            <BulletList>
          ...
          <Section id="service-providers" title="Service providers">
            <details className="group border-y border-[var(--color-divider)] py-4">
              <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-4 text-base font-semibold text-[var(--color-text)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-focus)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--color-canvas)] [&::-webkit-details-marker]:hidden">
                ...
                <span aria-hidden="true" className="text-sm text-[var(--color-text-muted)] group-open:hidden">Show</span>
                <span aria-hidden="true" className="hidden text-sm text-[var(--color-text-muted)] group-open:inline">Hide</span>
              </summary>
